...
if __name__ == '__main__':
    ...
    tf.reset_default_graph()
    print(
        "-----------------------------learning_rate: {}----------------num_factor: {}--------------num_factor_mlp: {}---------------------------"
    ...
    with tf.Session(config=config) as sess:
        ...
        if args.model == "NCPL":
            num_users, train_num_items, train_u_input, train_i_input, train_j_input = get_pairwise_train_dataset(
                path='data/%d_top%d/%s_train.dat' %
                (n, topK, dataset))

            print(
                "Number of users: {} and train_items: {}"
                .format(num_users, train_num_items))
            # num_users, num_items = 943, 1683
            # num_users, num_items = 9649, 17770

            train_labels = [
                1 for _ in range(len(train_u_input))
            ]
            print("Length of train_u_input: %d " %
                    len(train_u_input))
            ...
            print("Number of test_items: ",
                    test_num_items)
            num_items = max(train_num_items,
                            test_num_items)
            ...
            model = NCPL(
                ...
                ranker_layers=ranker_layers)

            model.prepare_data(train_u_input,
                                train_i_input,
                                train_j_input,
                                train_labels, testItems,
                                testRatings)
