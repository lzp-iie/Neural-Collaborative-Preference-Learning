...
class NCPL(object):
    def __init__(self,
        ...
        print("NCPL model has been initialized! ")
    ...
    def build_network(self):
        ...
        self.pred_y = tf.layers.dense(
            inputs=tf.concat([_GMF, mlp_layer_4], axis=1),
            units=1,
            # activation=tf.nn.relu,
            kernel_initializer=tf.random_normal_initializer,
            kernel_regularizer=tf.contrib.layers.l2_regularizer(
        ...
        self.bpr_ranking_loss = tf.reduce_sum(
            tf.negative(
                tf.log(1e-10 + tf.sigmoid(
                    tf.subtract(1 + self.pred_pos_y, self.pred_neg_y)))), 0)
    ...
    def execute(self):
        ...
        log_file = 'results/all/%d_%s_top%d.txt' % (now, self.dataset, self.topK)
